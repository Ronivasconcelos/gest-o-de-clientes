...
function exportPdf(){
  ...
  if(!report){showToast('Pop-up bloqueado. No Chrome, clique no ícone de pop-up bloqueado na barra de endereço e escolha Permitir para este site.','error');return;}
  report.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Backup de Clientes - ${todayLocal()}</title><style>
    *{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;color:#0f172a;margin:0;padding:28px;background:#fff}h1{margin:0 0 4px;font-size:24px}h2{margin:28px 0 10px;font-size:16px}.muted{color:#64748b;font-size:12px}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:18px 0}.card{border:1px solid #e2e8f0;border-radius:10px;padding:12px;background:#f8fafc}.label{display:block;font-size:10px;text-transform:uppercase;color:#64748b;font-weight:bold;margin-bottom:5px}.value{font-size:18px;font-weight:bold}.table-wrap{overflow:visible}table{width:100%;border-collapse:collapse;font-size:9px}th,td{border:1px solid #dbe3ec;padding:6px;text-align:left;vertical-align:top}th{background:#eef2ff;color:#1e3a8a;font-size:9px}.money{text-align:right;white-space:nowrap}.footer{margin-top:18px;font-size:10px;color:#64748b;border-top:1px solid #e2e8f0;padding-top:10px}@media print{body{padding:12mm}.no-print{display:none}.summary{break-inside:avoid}table{page-break-inside:auto}tr{page-break-inside:avoid;page-break-after:auto}thead{display:table-header-group}}
  </style></head><body><h1>Backup de Dados — Gestão de Clientes</h1><div class="muted">Gerado em ${formatDate(todayLocal())} • ${total} cliente(s)</div><div class="summary"><div class="card"><span class="label">Total</span><span class="value">${total}</span></div><div class="card"><span class="label">Pagos</span><span class="value">${paid}</span></div><div class="card"><span class="label">Pendentes</span><span class="value">${unpaid}</span></div><div class="card"><span class="label">Receita mensal</span><span class="value">${htmlEscapeForReport(formatCurrency(revenue))}</span></div></div><h2>Clientes e situação financeira</h2><div class="table-wrap"><table><thead><tr><th>Cliente</th><th>E-mail</th><th>Telefone</th><th>Empresa</th><th>Categoria</th><th>Status</th><th>Mensalidade</th><th>Total pago</th><th>Total pendente</th></tr></thead><tbody>${rows||'<tr><td colspan="9">Nenhum cliente cadastrado.</td></tr>'}</tbody></table></div><div class="footer">Este PDF é um relatório de backup para consulta e impressão. Os dados completos do sistema continuam armazenados no navegador deste dispositivo.</div><script>window.addEventListener('load',()=>setTimeout(()=>window.print(),350));<\/script></body></html>`);
